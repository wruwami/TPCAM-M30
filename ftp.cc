...
std::string ftp_t::get_file_path()
{
    char buf_request[255];
    std::string str_server_ip;
    std::string str_rsp;
//    unsigned short server_port = 21;
    ...
    std::string ret = parse_PWD_response(str_rsp);
...
std::string ftp_t::parse_PWD_response(const std::string &str_rsp)
{
  unsigned int h[4];
  unsigned int p[2];

  size_t pos = str_rsp.find('"');
  size_t pos2 = str_rsp.rfind('"');
  std::string str_pwd = str_rsp.substr(pos + 1, pos2 - pos);
