...
void print_wifi_printer(char *fullfilename)
{
    ...
    if (!strncmp(g_file_elem_for_printer.prefix, "AI", 2) || !strncmp(g_file_elem_for_printer.prefix, "AV", 2) || !strncmp(g_file_elem_for_printer.prefix, "MC", 2))
    {
        ...
        ************/
        ...
        char str_wifi_printer[128] = { 0 };
        json_data_manager_get_wifi_printer(str_wifi_printer);
            ...
            else
            {
                if (g_bOnlyComlaser)
                {
                    ...
                    if (g_is_print_on_logo)
                    {
                        ...
                        print_wifi_image_logo(g_wifi_printer.socket, atoi(str_wifi_printer));	// LOGO + Title (1712x320)
                        print_wifi_image(g_wifi_printer.socket, atoi(str_wifi_printer));	// 0 : Woosim, 1 : Eastroyce
...
void SetPrinterModelAndSize()
{
    char str_wifi_printer[128] = { 0 };
    json_data_manager_get_wifi_printer(str_wifi_printer);
    int nManufacturer = atoi(str_wifi_printer);
...
int connect_wifi_printer()
{
    ...
    char strIP[128] = { 0, };
    json_data_manager_get_ip_address_1(strIP1);
    json_data_manager_get_ip_address_2(strIP2);
    json_data_manager_get_ip_address_3(strIP3);
    sprintf(strIP, "%d.%d.%d.19", atoi(strIP1), atoi(strIP2), atoi(strIP3));
    //fprintf(stdout, "\n=== IP : %s\n\n", strIP);
